...
class PlatformSim:
    def __init__(self, args):
      ...
      self.comms_cmd_ack_writer = dds.DynamicData.DataWriter(
          self.comms_cmd_ack_topic,
          self.qos_provider.datawriter_qos_from_profile(args.qos_profile)
      )
      self.comms_status_writer = dds.DynamicData.DataWriter(
    ...
    async def read_comms_cmd_data(self):
      ...
      async for data in self.comms_cmd_reader.take_data_async():
    ...
    async def write_comms_cmd_ack(self):
      ...
      while True:
          self.comms_cmd_ack_writer.write(cmd_ack_sample)
          print("Writing to CommsCommandAck topic")
          await asyncio.sleep(1)

    async def write_comms_status(self):
      # Create sample
      comms_status_sample = dds.DynamicData(self.comms_status_type)
      ...
      while True:
          self.comms_status_writer.write(comms_status_sample)
